...
class PXEBootTestCase(test_common.CIMCBaseTestCase):
    ...
    @mock.patch.object(boot.network_provider, 'remove_provisioning_network',
                       autospec=True)
    @mock.patch.object(boot.network_provider, 'configure_tenant_networks',
                       autospec=True)
    @with_task
    def test_prepare_instance(self, task, mock_config, mock_unplug,
        ...
        mock_config.assert_called_once_with(task)
    ...
    @mock.patch.object(boot.network_provider, 'remove_provisioning_network',
                       autospec=True)
    @with_task
    def test_clean_up_ramdisk(self, task, mock_remove, mock_clean):
    ...
    @mock.patch.object(boot.network_provider, 'unconfigure_tenant_networks',
                       autospec=True)
    @with_task
    def test_clean_up_instance(self, task, mock_unconfig, mock_clean):
        task.driver.boot.clean_up_instance(task)
        mock_clean.assert_called_once_with(mock.ANY, task)
        mock_unconfig.assert_called_once_with(task)
