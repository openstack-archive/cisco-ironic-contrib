...
class CIMCAndInspectorInspect(base.InspectInterface):
    ...
    def inspect_hardware(self, task):
        ...
        with cimc_common.cimc_handle(task) as handle:
            ...
            inspect_nic1 = handle.get_imc_managedobject(
                None, None,
                params={"Dn": "%s/host-eth-eth0" % adaptorunits[0].Dn})[0]
            inspect_nic2 = handle.get_imc_managedobject(
                None, None,
                params={"Dn": "%s/host-eth-eth1" % adaptorunits[0].Dn})[0]
        ...
        net = client.show_network(CONF.provisioning_network_uuid)
        seg_id = net['network']['provider:segmentation_id']

        # Configure vnic for the provisioning network but without a neutron
        # port so that the ironic inspector boots the node.
        common.add_vnic(task, 0, inspect_nic1.Mac, seg_id,
                        pxe=True, trunk=True)
        common.add_vnic(task, 1, inspect_nic2.Mac, seg_id,
                        pxe=True, trunk=True)
        ...
        for node_uuid, driver in node_iter:
            try:
                ...
                with task_manager.acquire(context, node_uuid, shared=True,
                    ...
                    if error:
                        self._finish_inspection(task, fail=True)
    ...
    def _finish_inspection(self, task, fail=False):
        ...
        # If introspection failed don't bother setting the driver_info
        if not fail:
            task.node.driver_info = driver_info
            task.node.save()
        task.ports = objects.Port.list_by_node_id(task.context, task.node.id)
